...
--SP2
--Details: Stored procedures to display all the tables from the database.
--Created by: Ashish Kenjale
...
BEGIN
/*
	DBMS_OUTPUT.PUT_LINE('In Package ENROLL Trigger...!');
	DBMS_OUTPUT.PUT_LINE('New Classid : ' || :new.CLASSID);
	DBMS_OUTPUT.PUT_LINE('Old Classid : ' || :old.CLASSID);
*/
	CASE
    		WHEN INSERTING THEN
...
FOR EACH ROW
BEGIN
/*	DBMS_OUTPUT.PUT_LINE('In Students Trigger...!');
	DBMS_OUTPUT.PUT_LINE('New Classid : ' || :new.B#);
	DBMS_OUTPUT.PUT_LINE('Old Classid : ' || :old.B#);
*/
	CASE
		WHEN DELETING THEN
